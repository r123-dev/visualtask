...
const Flavonoids = () => {
    ...
    //   const means = Object.keys(groupedData).map(key => {
    //     const flavanoidsSum = groupedData[key].reduce((sum, obj) => sum + obj.Flavanoids, 0);
    //     const mean = flavanoidsSum / groupedData[key].length;
    //     return { Alcohol: Number(key), MeanFlavanoids: mean.toFixed(3) };
    //   });
    
    const means = [];
    for (let alcoholType = 1; alcoholType <= 2; alcoholType++) {
        const filteredData = arr.filter(obj => obj.Alcohol === alcoholType);
        if (filteredData.length > 0) {
          const flavanoidsSum = filteredData.reduce((sum, obj) => sum + obj.Flavanoids, 0);
          const mean = flavanoidsSum / filteredData.length;
          means.push({ Alcohol: alcoholType, MeanFlavanoids: mean.toFixed(3) });
        } else {
          means.push({ Alcohol: alcoholType, MeanFlavanoids: "No data" });
        }
      }
      const map={};
      const filteredData = arr.filter(obj => obj.Alcohol === 3);
      if (filteredData.length > 0) {
        const flavanoidsSum = filteredData.reduce((sum, obj) => sum + parseFloat(obj.Flavanoids), 0);
        const mean = flavanoidsSum / filteredData.length;
        means.push({ Alcohol: 3, MeanFlavanoids: mean.toFixed(3) });
      }
      for(let i=0;i<means.length;i++)
      {
        map[means[i].Alcohol]=means[i].MeanFlavanoids;
      }
      const groupedData = arr.reduce((acc, obj) => {
        ...
        acc[key].push(obj.Flavanoids);
        return acc;
      }, {});
      
      // Calculate the mode of 'Flavanoids' for each 'Alcohol' type
      const modes = Object.keys(groupedData).map(key => {
        const counts = {};
        ...
        // If all occurrences are unique, set mode to NaN
        if (maxCount === 1) {
          mode = NaN;
        }
      
        return { Alcohol: Number(key), ModeFlavanoids: mode.toFixed(3) };
      });
      const medians = Object.keys(groupedData).map(key => {
        const sortedValues = groupedData[key].sort((a, b) => a - b);
        const len = sortedValues.length;
        let median;
      
        if (len % 2 === 0) {
          median = (sortedValues[len / 2 - 1] + sortedValues[len / 2]) / 2;
        } else {
          median = sortedValues[Math.floor(len / 2)];
        }
      
        return { Alcohol: Number(key), MedianFlavanoids: median.toFixed(3) };
      });
