...
const Gamma = () => {
   ...
   const arr= [
      ...
      arr.forEach(obj => {
        obj.Gamma = (obj.Ash * obj.Hue) / obj.Magnesium;
      });
      
      // Group data by 'Alcohol'
      const groupedData = arr.reduce((acc, obj) => {
        const key = obj.Alcohol;
        if (!acc[key]) {
          acc[key] = [];
        }
        acc[key].push(obj.Gamma);
        return acc;
      }, {});

      const statistics = Object.keys(groupedData).map(key => {
        ...
        values.forEach(val => {
          ...
          if (valueCount[val] > maxCount) {
            maxCount = valueCount[val];
            mode = val;
          }
        });
      
        return { Alcohol: Number(key), MeanGamma: mean.toFixed(3), MedianGamma: median.toFixed(3), ModeGamma: mode.toFixed(3) };
      });
  ...
  return (
    ...
    <table>
      ...
      <tbody>
          <tr>
             <td>Gamma Mean</td>
             <td>{statistics[0].MeanGamma}</td>
             <td>{statistics[1].MeanGamma}</td>
             <td>{statistics[2].MeanGamma}</td>
          </tr>
          <tr>
             <td>Gamma Mode</td>
             <td>{statistics[0].ModeGamma}</td>
             <td>{statistics[1].ModeGamma}</td>
             <td>{statistics[2].ModeGamma}</td>
          </tr>
          <tr>
             <td>Gamma Median</td>
             <td>{statistics[0].MedianGamma}</td>
             <td>{statistics[1].MedianGamma}</td>
             <td>{statistics[2].MedianGamma}</td>
          </tr>
      </tbody>
